...
class Observer():
    ...
    def _get_actions_old(self, fnames: list[str], include_transcript: bool = False):
        actions = []
        for fname in fnames:
            actions.append(Observer._load_markdown(f'/Users/dorazhao/Documents/modelgardens/src/infact_dataset/summaries/{self.index}/{fname}'))
            if include_transcript:
                actions.append(f"Transcription of User's Screen")
                actions.append(Observer._load_markdown(f'/Users/dorazhao/Documents/modelgardens/src/infact_dataset/transcripts/{self.index}/{fname}'))
        actions = '\n'.join(actions)
        return actions
    
    def _get_context(self, annotations, index):
            """
            Efficient version using binary search.
            """
            sorted_keys = sorted([int(k) for k in annotations.keys()])
            
            # Find insertion point
            pos = bisect.bisect_right(sorted_keys, index)
            
            if pos > 0:
                context_key = sorted_keys[pos - 1]
                return annotations[str(context_key)]
            return None
            
    def _get_actions(self, fnames: list[str], include_transcript: bool = False):
        actions = []
        for fname in fnames:
            actions.append(Observer._load_markdown(f'../data/{self.index}/processed_data/summaries/{fname}'))
            if include_transcript:
                actions.append(f"Transcription of User's Screen")
                actions.append(Observer._load_markdown(f'../data/{self.index}/processed_data/transcripts/{fname}'))
        actions = '\n'.join(actions)
        return actions
    ...
    def _handle_similar(self, new_obs:dict, targets: list[str]):
        new_evidence = new_obs['evidence']
        for tid_ in targets:
            tid_ = str(tid_)
            tgt = self.all_actions[tid_]
    ...
    async def observer_pipeline_by_session(self, input_dir, end_file=-1, include_transcript=False, window_size=5):
        ...
        sessions = {}
        for file in files:
            session_num = os.path.splitext(file)[0].split("_")[0]
            if session_num not in sessions:
                sessions[session_num] = []
            sessions[session_num].append(file)
        sessions = OrderedDict(sorted(sessions.items()))
        ...
        for name in sessions: # REMOVE: RUN ONLY FIRST SESSION
            if name == "session-0": continue
            session = sessions[name]
            session_length = len(session)
            context_file = f"../data/{self.index}/pipeline_outputs/{name}/annotated_data.json"
            exists_context = os.path.exists(context_file)
            if exists_context:
                context_annotations = json.load(open(context_file, "r"))
            else:
                context_annotations = {}
            ...
            for index in range(0, session_length, window_size):
                try:
                    ...
                    # ----- Load and propose -----
                    actions = self._get_actions(fnames, include_transcript=False)
                    
                
                    if exists_context:
                        context = self._get_context(context_annotations, index)
                        input_prompt = observer.OBSERVE_CONTEXT_PROMPT.format(context=context, actions=actions, user_name=self.name)
                    else:
                    ...
                    for prop in new_needs.observations:
                        ...
                        self._handle_different(item, prop.description)
            ...
            # Save at the end of each session and clear BM25
            save_dir = f"../data/{self.index}/pipeline_outputs/{name}"
            os.makedirs(save_dir, exist_ok=True)
            save_file = f"{save_dir}/observations_feelings_{self.model}_conservative.json"
            with open(save_file, "wb") as f:
                f.write(orjson.dumps(self.all_actions, option=orjson.OPT_INDENT_2))
            # clear BM25
            self.actions_index.clear()
            self.all_actions = {}
            assert len(self.actions_index.needs) == 0
    ...
    async def observer_pipeline(self, input_dir, end_file=-1, include_transcript=False, window_size=5):
        ...
        for name in sessions:
            ...
            # Save at the end of each session and clear BM25
            save_dir = f"../data/${self.index}/llm_outputs/observations"
            os.makedirs(save_dir, exist_ok=True)
            save_file = f"{save_dir}/observations_feelings_{self.model}.json"
            with open(save_file, "wb") as f:
                f.write(orjson.dumps(self.all_actions, option=orjson.OPT_INDENT_2))
            # clear BM25
            if end_session: break
...
async def main(args):
    save_file = f"../data/{args.index}/llm_outputs/observations_{args.model}_updated-prompt_session0.json"
    t = Observer(args.model, str(args.index), args.user, save_file=save_file)
    input_dir = f"../data/{args.index}/processed_data/transcripts"
    results = await t.observer_pipeline_by_session(input_dir, end_file=args.end_file, include_transcript=True)
    # with open(save_file, "wb") as f:
    #     f.write(orjson.dumps(results, option=orjson.OPT_INDENT_2))

if __name__ == "__main__":
    # data = json.load(open("../data/dora_pilot/pipeline_outputs/session-3/observations_feelings_gpt-4.1_conservative.json", "r"))
    # print("WHAT DORA IS FEELING:")
    # for d in data:
    #     print("- " + data[d]['description'])
    
    # print("WHAT DORA IS DOING:")
    # for d in data:
    #     print("- " + data[d]['evidence'][0])
        # print("ID: {} | Feeling: {} | Action: {}".format(d, data[d]["description"], data[d]["evidence"][0]))
    ...
    parser.add_argument("--include_transcript", type=bool, default=True)
...
#     "insights": [
#         {
#             "insight": "1-2 sentences describing the insight",
#             "evidence": "Specific evidence from the input that support the insight. Direct quotes from the input are preferred.",
