...
class Pipeline():
    ...
    async def watch_directory(self, interval: float = 1.0):
        print("📡 Starting background watcher...")
        while True:
            all_frames = sorted([
                f for f in os.listdir(self.file_directory)
                if f.startswith("frame_") and f.endswith(".jpg")
            ])
            total_frames = len(all_frames)
            
            # Proceed if enough new frames are available
            if total_frames >= self.start_frame + self.fp and not self.start_processing:
                print(f"🟢 Detected enough frames. Processing from frame {self.start_frame}")
    ...
    async def process_actions(self):
        """
            Transcribes and summarizes the files in directory. Assumes that files are
            named in a sequential order of the format "frame_0", "frame_1"
        """
        processor = ImageProcessor()
        image_paths = []
        new_max = self.max_frame
        for i in range(self.start_frame, self.start_frame + self.fp):
            path = self.file_directory + 'frame_{}.jpg'.format(i)
            if not os.path.exists(path):
                print(f"⚠️ Missing frame: {path}")
                continue
            image_paths.append(path)
            new_max = i
        if new_max == self.max_frame:
            self.can_process = False 
            return '', ''
        else:
            self.max_frame = new_max
            tasks = [processor.call_gpt_vision(prompt = screen.TRANSCRIPTION_PROMPT, images = image_paths, id = self.start_frame), \
                    processor.call_gpt_vision(prompt = screen.SUMMARY_PROMPT, images = image_paths, id = self.start_frame)]
            res = await asyncio.gather(*tasks)
            transcription, summary= res[0], res[1]
            os.makedirs(f'transcripts/workflow_{self.workflow}', exist_ok=True)
            with open('transcripts/workflow_{}/{}.md'.format(self.workflow, self.start_frame), 'w') as f:
                f.write(transcription[0])
            with open('transcripts/workflow_{}/{}_summary.md'.format(self.workflow, self.start_frame), 'w') as f:
                f.write(summary[0])
            return transcription[0], summary[0]
...
if __name__ == '__main__':
    pipeline = Pipeline('45', 'sampled_frames/workflow_45/', True)
    # params = {
    #         'feasibility': 0.9,
    #         'utility': 1.1
    # }
